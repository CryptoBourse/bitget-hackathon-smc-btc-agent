...
class SmcGoldStrategy(Strategy):
    ...
    def _try_pending_entry(
        ...
        if self._pending_side == "LONG":
            ...
            choch_ok = close > self._last_swing_high or self._last_swing_high <= 0
            if touched_ob and in_value and choch_ok:
            ...
            choch_ok = close < self._last_swing_low or self._last_swing_low <= 0
            if touched_ob and in_value and choch_ok:
                qty = Quantity(Decimal(self.cfg.trade_size), instrument.size_precision)
